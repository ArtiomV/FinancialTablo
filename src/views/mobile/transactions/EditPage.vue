<template>
    <f7-page @page:afterin="onPageAfterIn" @page:beforeout="onPageBeforeOut">
        ...
        <f7-list strong inset dividers class="margin-vertical skeleton-text" v-if="loading">
            ...
            <!-- Hidden: Transaction Time skeleton (user requested to hide time field) -->
            <f7-list-item class="list-item-with-header-and-title" header="Transaction Time" title="YYYY/MM/DD HH:mm:ss" v-if="false && pageTypeAndMode?.type === TransactionEditPageType.Transaction"></f7-list-item>
            ...
            <!-- Hidden: Transaction Timezone skeleton (user requested to hide time field) -->
            <f7-list-item class="list-item-with-header-and-title list-item-title-hide-overflow list-item-no-item-after" header="Transaction Timezone" title="(UTC XX:XX) System Default" link="#" :no-chevron="mode === TransactionEditPageMode.View" v-if="false && (pageTypeAndMode?.type === TransactionEditPageType.Transaction || (pageTypeAndMode?.type === TransactionEditPageType.Template && transaction instanceof TransactionTemplate && (transaction as any).templateType === TemplateType.Schedule.type))"></f7-list-item>
            <!-- Hidden: Geographic Location skeleton (user requested to hide location field) -->
            <f7-list-item class="list-item-with-header-and-title list-item-title-hide-overflow" header="Geographic Location" title="No Location" v-if="false && pageTypeAndMode?.type === TransactionEditPageType.Transaction"></f7-list-item>
        ...
        <f7-list form strong inset dividers class="margin-vertical" v-else-if="!loading">
            ...
            >
                <template #title>
                    <div class="list-item-custom-title" v-if="hasVisibleExpenseCategories">
                        <span>{{ getCategoryNameById(transaction.expenseCategoryId, allCategories[CategoryType.Expense]) }}</span>
                    </div>
            ...
            >
                <template #title>
                    <div class="list-item-custom-title" v-if="hasVisibleIncomeCategories">
                        <span>{{ getCategoryNameById(transaction.incomeCategoryId, allCategories[CategoryType.Income]) }}</span>
                    </div>
            ...
            >
                <template #title>
                    <div class="list-item-custom-title" v-if="hasVisibleTransferCategories">
                        <span>{{ getCategoryNameById(transaction.transferCategoryId, allCategories[CategoryType.Transfer]) }}</span>
                    </div>
            ...
            <f7-list-item
                ...
                v-if="false && (pageTypeAndMode?.type === TransactionEditPageType.Transaction || (pageTypeAndMode?.type === TransactionEditPageType.Template && transaction instanceof TransactionTemplate && (transaction as any).templateType === TemplateType.Schedule.type))"
            ...
            <f7-list-item
                ...
                link="#" no-chevron
                :class="{ 'readonly': mode === TransactionEditPageMode.View }"
                :header="tt('Counterparty')"
                :title="selectedCounterpartyName"
                @click="showCounterpartySheet = true"
                v-if="transaction.type !== TransactionType.ModifyBalance"
            >
                <list-item-selection-sheet value-type="item"
                                           key-field="id" value-field="id" title-field="name"
                                           :items="counterpartyItemsWithNone"
                                           v-model:show="showCounterpartySheet"
                                           v-model="transaction.counterpartyId">
                </list-item-selection-sheet>
            </f7-list-item>

            <!-- Hidden: Pictures block (user requested to hide pictures block) -->
            <f7-list-item
                link="#" no-chevron
                :header="tt('Pictures')"
                v-if="false && (showTransactionPictures || (transaction.pictures && transaction.pictures!.length > 0))"
            >
                <template #footer>
            ...
            <f7-list-item v-if="pageTypeAndMode?.type === TransactionEditPageType.Transaction && mode === TransactionEditPageMode.Add">
                <template #title>
                    <span>{{ tt('Repeatable') }}</span>
                </template>
                <template #after>
                    <f7-toggle :checked="isRepeatable" @toggle:change="isRepeatable = $event"></f7-toggle>
                </template>
            </f7-list-item>

            <f7-list-item link="#" no-chevron
                :header="tt('Scheduled Transaction Frequency')"
                v-if="isRepeatable && pageTypeAndMode?.type === TransactionEditPageType.Transaction && mode === TransactionEditPageMode.Add"
                @click="showRepeatFrequencySheet = true">
                <template #after>
                    <span>{{ repeatFrequencyDisplayName }}</span>
                </template>
            </f7-list-item>

            <schedule-frequency-sheet
                v-model:show="showRepeatFrequencySheet"
                v-model:type="repeatFrequencyType"
                v-model="repeatFrequency"
                v-if="isRepeatable && pageTypeAndMode?.type === TransactionEditPageType.Transaction && mode === TransactionEditPageMode.Add" />
        </f7-list>

        <!-- Hidden: Geographic Location action sheet (user requested to hide location field) -->
        <f7-actions v-if="false" close-by-outside-click close-on-escape :opened="showGeoLocationActionSheet" @actions:closed="showGeoLocationActionSheet = false">
            <f7-actions-group>
        ...
        <f7-toolbar tabbar bottom v-if="mode === TransactionEditPageMode.View && transaction.planned">
            <f7-link :class="{ 'disabled': loading || submitting }" @click="confirmPlanned">
                <span class="tabbar-primary-link">{{ tt('Confirm') }}</span>
                <f7-preloader :size="20" v-if="submitting" />
            </f7-link>
        </f7-toolbar>
...
import { getCategoryNameById } from '@/lib/category.ts';
...
const showCounterpartySheet = ref<boolean>(false);
...
const showRepeatFrequencySheet = ref<boolean>(false);
...
const selectedCounterpartyName = computed<string>(() => {
    if (transaction.value.counterpartyId && transaction.value.counterpartyId !== '0') {
        const counterparty = counterpartiesStore.allCounterpartiesMap[transaction.value.counterpartyId];
        if (counterparty) {
...
const counterpartyItemsWithNone = computed(() => {
    const noneItem = { id: '0', name: tt('None'), hidden: false };
    return [noneItem, ...counterpartiesStore.allVisibleCounterparties];
});

const repeatFrequencyDisplayName = computed<string>(() => {
    const freqType = ScheduledTemplateFrequencyType.valueOf(repeatFrequencyType.value);
    return freqType ? tt(freqType.name) : '';
});
                ...
                if (mode.value === TransactionEditPageMode.Edit && transaction.value.planned && transaction.value.sourceTemplateId && transaction.value.sourceTemplateId !== '0') {
                    showConfirm(tt('Do you want to apply these changes to all future planned transactions?'), () => {
                        ...
                        services.modifyAllFuturePlannedTransactions({
                            ...
                            if (!error.processed) {
                                showToast('Unable to modify future planned transactions');
                            }
...
function confirmPlanned(): void {
    if (!transaction.value || !transaction.value.id) {
        return;
    }

    showConfirm(
        tt('Are you sure you want to confirm this planned transaction? The transaction date will be set to today.'),
        () => {
            submitting.value = true;
            ...
            services.confirmPlannedTransaction({ id: transaction.value.id }).then(response => {
                ...
                showToast(tt('Unable to confirm planned transaction'));
