    ...
    <f7-page ptr
        ...
        <f7-navbar>
            ...
            <f7-nav-right :class="{ 'navbar-compact-icons': true, 'disabled': loading }">
                <f7-link :icon-f7="showPlannedTransactions ? 'eye_slash' : 'eye'" @click="showPlannedTransactions = !showPlannedTransactions"></f7-link>
                        ...
                        >
                                        ...
                                        <div class="item-title-row">
                                            <div class="item-title">
                                                <div class="transaction-category-name no-padding">
                                                    ...
                                                    <f7-badge color="orange" class="margin-left-half" v-if="transaction.planned">{{ tt('Planned') }}</f7-badge>
                                                </div>
                                            </div>
                                            <div class="item-after">
                            ...
                            <f7-swipeout-actions :left="textDirection === TextDirection.RTL"
                                ...
                                <f7-swipeout-button color="green" close
                                                    :text="tt('Confirm')"
                                                    :class="{ 'disabled': confirmingPlannedTransaction }"
                                                    v-if="transaction.planned"
                                                    @click="confirmPlannedTransaction(transaction)"></f7-swipeout-button>
                ...
                <template :key="categoryType"
                    ...
                    <template v-if="categories && categories.length">
                        ...
                        <f7-list-item link="#" no-chevron popover-close
                                      ...
                                      v-show="!category.hidden || queryAllFilterCategoryIds[category.id]"
...
const transactions = computed<TransactionMonthList[]>(() => {
    ...
    const filterPlanned = (monthLists: TransactionMonthList[]): TransactionMonthList[] => {
        if (showPlannedTransactions.value) {
            return monthLists;
        }

        return monthLists.map(ml => ({
            ...ml,
            items: ml.items.filter(t => !t.planned)
        })).filter(ml => ml.items.length > 0);
    };

    if (pageType.value === TransactionListPageType.List.type) {
        return filterPlanned(transactionsStore.transactions);
        ...
        if (queryMonthlyData.value) {
            ...
            return filterPlanned([dailyTransactionList]);
        } else {
            return [];
        }
    } else {
...
const grandTotalDisplay = computed<{ income: string, expense: string } | null>(() => {
    ...
    return {
        income: getDisplayMonthTotalAmount(grandTotal.income, defaultCurrency.value, '', grandTotal.incompleteIncome),
        expense: getDisplayMonthTotalAmount(grandTotal.expense, defaultCurrency.value, '', grandTotal.incompleteExpense)
    };
});
