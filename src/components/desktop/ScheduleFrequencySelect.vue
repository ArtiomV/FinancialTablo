        ...
        <template #no-data>
            <div ref="dropdownMenu" class="schedule-frequency-container">
                ...
                <div class="schedule-frequency-value-container">
                    ...
                    <v-list select-strategy="classic" v-model:selected="frequencyValue"
                            v-else-if="frequencyType === ScheduledTemplateFrequencyType.Monthly.type || frequencyType === ScheduledTemplateFrequencyType.Bimonthly.type || frequencyType === ScheduledTemplateFrequencyType.Quarterly.type || frequencyType === ScheduledTemplateFrequencyType.Semiannually.type || frequencyType === ScheduledTemplateFrequencyType.Annually.type">
        ...
        if (props.type !== value) {
            ...
            if (value === ScheduledTemplateFrequencyType.Weekly.type) {
                frequencyValue.value = [firstDayOfWeek.value];
                       ...
                       value === ScheduledTemplateFrequencyType.Bimonthly.type ||
                       value === ScheduledTemplateFrequencyType.Quarterly.type ||
                       value === ScheduledTemplateFrequencyType.Semiannually.type ||
                       value === ScheduledTemplateFrequencyType.Annually.type) {
...
const displayFrequency = computed<string>(() => {
    ...
    } else if (frequencyType.value === ScheduledTemplateFrequencyType.Bimonthly.type) {
        const freqName = tt('Every 2 Months');
        if (frequencyValue.value.length) {
            return freqName + ', ' + getMultiMonthdayShortNames(frequencyValue.value);
        }
        return freqName;
    } else if (frequencyType.value === ScheduledTemplateFrequencyType.Quarterly.type) {
        const freqName = tt('Quarterly');
        if (frequencyValue.value.length) {
            return freqName + ', ' + getMultiMonthdayShortNames(frequencyValue.value);
        }
        return freqName;
    } else if (frequencyType.value === ScheduledTemplateFrequencyType.Semiannually.type) {
        const freqName = tt('Every 6 Months');
        if (frequencyValue.value.length) {
            return freqName + ', ' + getMultiMonthdayShortNames(frequencyValue.value);
        }
        return freqName;
    } else if (frequencyType.value === ScheduledTemplateFrequencyType.Annually.type) {
        const freqName = tt('Annually');
        if (frequencyValue.value.length) {
            return freqName + ', ' + getMultiMonthdayShortNames(frequencyValue.value);
        }
        return freqName;
