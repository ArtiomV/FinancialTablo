...
export const useTransactionsStore = defineStore('transactions', () => {
    ...
    const grandTotalAmount = computed<TransactionTotalAmount>(() => {
        let totalExpense = 0;
        let totalIncome = 0;
        let incompleteExpense = transactionsNextTimeId.value > 0;
        let incompleteIncome = transactionsNextTimeId.value > 0;

        for (const monthList of transactions.value) {
            totalExpense += monthList.totalAmount.expense;
            totalIncome += monthList.totalAmount.income;

            if (monthList.totalAmount.incompleteExpense) {
                incompleteExpense = true;
            }

            if (monthList.totalAmount.incompleteIncome) {
    ...
    function saveTransaction({ transaction, defaultCurrency, isEdit, clientSessionId, repeatOptions }: { transaction: Transaction, defaultCurrency: string, isEdit: boolean, clientSessionId: string, repeatOptions?: { repeatable: boolean, repeatFrequencyType: number, repeatFrequency: string } }): Promise<Transaction> {
        return new Promise((resolve, reject) => {
            ...
            if (!isEdit) {
                promise = services.addTransaction(transaction.toCreateRequest(clientSessionId, repeatOptions));
            } else {
