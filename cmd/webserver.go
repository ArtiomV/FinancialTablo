...
func startWebServer(c *core.CliContext) error {
	...
	apiRoute.Use(bindMiddleware(middlewares.RequestLog))
	{
		...
		apiV1Route.Use(bindMiddleware(middlewares.JWTAuthorization(config)))
		{
			...
			apiV1Route.POST("/transactions/confirm.json", bindApi(api.Transactions.TransactionConfirmHandler))
			apiV1Route.POST("/transactions/modify_all_future.json", bindApi(api.Transactions.TransactionModifyAllFutureHandler))
			...
			apiV1Route.POST("/transactions/delete_all_future.json", bindApi(api.Transactions.TransactionDeleteAllFutureHandler))
			apiV1Route.POST("/transactions/set_planned.json", bindApi(api.Transactions.TransactionSetPlannedHandler))
			apiV1Route.POST("/transactions/make_repeatable.json", bindApi(api.Transactions.TransactionMakeRepeatableHandler))
			...
			// Counterparties
			apiV1Route.GET("/transaction/counterparties/list.json", bindApi(api.Counterparties.CounterpartyListHandler))
			apiV1Route.GET("/transaction/counterparties/get.json", bindApi(api.Counterparties.CounterpartyGetHandler))
			apiV1Route.POST("/transaction/counterparties/add.json", bindApi(api.Counterparties.CounterpartyCreateHandler))
			apiV1Route.POST("/transaction/counterparties/modify.json", bindApi(api.Counterparties.CounterpartyModifyHandler))
