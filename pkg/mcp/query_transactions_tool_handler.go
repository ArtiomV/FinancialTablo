...
type MCPQueryTransactionsRequest struct {
	...
	CategoryName string `json:"category_name,omitempty" jsonschema_description:"Category name to filter transactions by (optional)"`
...
type MCPTransactionInfo struct {
	...
	CategoryName           string `json:"category_name,omitempty" jsonschema_description:"Category name for the transaction"`
...
func (h *mcpQueryTransactionsToolHandler) Handle(c *core.WebContext, callToolReq *MCPCallToolRequest, user *models.User, currentConfig *settings.Config, services MCPAvailableServices) (any, []*MCPTextContent, error) {
	...
	allCategories, err := services.GetTransactionCategoryService().GetAllCategoriesByUid(c, uid, 0)
	...
	if queryTransactionsRequest.CategoryName != "" {
		filterCategoryIds = services.GetTransactionCategoryService().GetCategoryOrSubCategoryIdsByCategoryName(allCategories, queryTransactionsRequest.CategoryName)
	...
	queryParams := &models.TransactionQueryParams{
		...
		Keyword:            queryTransactionsRequest.Keyword,
	}

	totalCount, err := services.GetTransactionService().GetTransactionCount(c, queryParams)
	...
	transactions, err := services.GetTransactionService().GetTransactionsByMaxTime(c, &models.TransactionQueryParams{
		Uid:                uid,
		MaxTransactionTime: maxTransactionTime,
		MinTransactionTime: minTransactionTime,
		TransactionType:    transactionType,
		...
		AccountIds:         filterAccountIds,
		Keyword:            queryTransactionsRequest.Keyword,
		Page:               queryTransactionsRequest.Page,
		Count:              queryTransactionsRequest.Count,
		NoDuplicated:       true,
	})
	...
	for i := 0; i < len(transactions); i++ {
		...
		if _, exists := filteredFields["category_name"]; exists || len(filteredFields) == 0 {
			if category, exists := categoriesMap[transaction.CategoryId]; exists && category != nil {
				transactionInfo.CategoryName = category.Name
			}
		}
